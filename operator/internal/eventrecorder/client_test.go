...
func withFakeClock(t *testing.T, client *Client) *time.Time {
	t.Helper()
	now := time.Now()
...
func TestEvent_UnmatchedReasonNotRateLimited(t *testing.T) {
	delegate := record.NewFakeRecorder(10)
	client := New(delegate, testConfig())
	withFakeClock(t, client)
	obj := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: "test"}}
	...
	for i := 0; i < 3; i++ {
		client.Eventf(obj, corev1.EventTypeNormal, "OtherReason", "msg %d", i)
		event, ok := drainEvent(t, delegate)
		assert.True(t, ok, "expected event on iteration %d", i)
		assert.Equal(t, fmt.Sprintf("Normal OtherReason msg %d", i), event)
	}
}

func TestEvent_WarningNotRateLimited(t *testing.T) {
	delegate := record.NewFakeRecorder(10)
	client := New(delegate, testConfig())
	withFakeClock(t, client)
	obj := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: "test"}}
	...
	for i := 0; i < 3; i++ {
		client.Eventf(obj, corev1.EventTypeWarning, "Synced", "warn %d", i)
		_, ok := drainEvent(t, delegate)
		assert.True(t, ok, "expected warning event on iteration %d", i)
	}
}
